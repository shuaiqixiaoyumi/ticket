...
function hide(){
	...
	document.getElementById("deldoc").style.display = "none";
}
function show(){
	document.getElementById("editdoc").style.display = "block";
	document.getElementById("deldoc").style.display = "block";
}
...
function display(result){
	...
	document.getElementById("remark").innerHTML = file_remark;
	document.getElementById("copyurl").value = copyurl;
	// 获取文章内容
	var insertText = artical.contents;
	// 填充文本框
	document.getElementById("test-editormd-view2").innerHTML = '<textarea id="to_load" style="display: none;">' + insertText + '</textarea>';
	// 解析
...
//	printStr =printStr+"<style type='text/css'>ul li {list-style-type:disc}</style>";
	...
	printStr = printStr + "</head><body ><div id='test-editormd-view2' class='markdown-body editormd-html-preview'>";
