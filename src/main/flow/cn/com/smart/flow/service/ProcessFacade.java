...
import cn.com.smart.flow.interceptor.AcceptHadnler;
...
public class ProcessFacade {
	...
	public SmartResponse<String> completeTask(SubmitFormData submitFormData,String userId,String orgId) {
		...
		try {
			...
			System.out.println("nextAssigners==="+nextAssigners);
	...
	public SmartResponse<OrgUserZTreeData> getAssignees(String processId,String orderId,String userId,String taskKey) {
		...
		if(StringUtils.isNotEmpty(processId) && StringUtils.isNotEmpty(taskKey)) {
			 ...
			 if(null != taskModel) {
				 ...
				 //暂时过滤用，后面改好接收人处理类后再替换
//				 if(processId.equals("8f4893f59ea7402291c8be973e2c622a") && taskKey.equals("rect3")) {
//					 AcceptHadnler ass = new AcceptHadnler();
//					 Execution ex = null;
//					 String accepts = ass.assignByParentUser(ex, userId, assignees).toString();
//					 if(accepts != null && !accepts.equals("")) {
//						 assignees = accepts;
//					 }
//				 }
//				 
				 if(taskModel.getAssignmentHandler()!=null) {
					 MyAssignmentHandler ass = SmartContextService.findByName(taskModel.getAssignmentHandler(), MyAssignmentHandler.class);
					 Execution ex = null;
					 String accepts = ass.assignByParentUser(ex, userId, assignees).toString();
					 if(accepts != null && !accepts.equals("")) {
						 assignees = accepts;
				 ...
				 taskModel.setAssignee(assignees);
