...
public class TableTreeTag extends AbstractTableTreeTag {
	...
	protected String getHtml(Boolean isParent,Object[] objArray, int row, int layer, String defaultValue, int startIndex, int cols, int headerCount) {
		...
		String clickfunction = null;
		if(isClick == 1) {
			clickfunction = "onclick =\""+clickfun+"(\'"+objArray[0]+"\')\"";
		}
		strBuff.append("<tr id='t-"+StringUtils.handleNull(objArray[0])+" ' "+clickfunction+" class='tr-tree "+classOpTree+" t-tree-layer"+layer+" t-"+StringUtils.handleNull(objArray[1])+"' parentid='t-"+StringUtils.handleNull(objArray[1])+"'>");
		int count = 0;
