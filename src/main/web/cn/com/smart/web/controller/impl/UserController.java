...
public class UserController extends BaseController {
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/flowsimplist")
    public ModelAndView flowsimplist(HttpSession session,UserSearchParam searchParam,
    		ModelAndView modelView,RequestPage page) throws Exception {
		String uri = "user/flowsimplist";
		searchParam.setOrgIds(StringUtils.list2Array(getUserInfoFromSession(session).getOrgIds()));
		SmartResponse<Object> smartResp = opServ.getDatas("user_simp_list",searchParam, page.getStartNum(), page.getPageSize());
		pageParam = new PageParam(uri, "#flowuser-tab", page.getPage(), page.getPageSize());
		selectedEventProp = new SelectedEventProp(SelectedEventType.OPEN_TO_TARGET.getValue(),"flowauth/userHas","#has-flowauth-list","id");	
		...
		modelView.setViewName(VIEW_DIR+"/flowsimplist");
		return modelView;
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/flowrolelist")
	public ModelAndView flowrolelist(UserSearchParam searchParam,ModelAndView modelView,RequestPage page) throws Exception {
		String uri = "user/flowrolelist";
		SmartResponse<Object> smartResp = opServ.getDatas("user_flowrole_list",searchParam, page.getStartNum(), page.getPageSize());
		pageParam = new PageParam(uri, null, page.getPage(), page.getPageSize());
		uri = uri+"?id="+searchParam.getId();
		addBtn = new EditBtn("add","user/addFlowRole?id="+searchParam.getId(), "用户中添加角色", "600");
		delBtn = new DelBtn("user/deleteFlowRole?userId="+searchParam.getId(), "确定要从该用户中删除选中的角色吗？",uri,"#user-flowrole-tab", null);
		refreshBtn = new RefreshBtn(uri, null,"#user-flowrole-tab");
		...
		modelView.setViewName(VIEW_DIR+"/flowrolelist");
		return modelView;
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/addFlowRole")
	public ModelAndView addFlowRole(UserSearchParam searchParam,ModelAndView modelView,RequestPage page) throws Exception {
		String uri = "user/addFlowRole";
		SmartResponse<Object> smartResp = opServ.getDatas("user_addflowrole_list",searchParam, page.getStartNum(), page.getPageSize());
		...
		modelMap.put("searchParam", searchParam);
		pageParam = null;
		modelView.setViewName(VIEW_DIR+"/addFlowRole");
		return modelView;
	}
	...
	public @ResponseBody SmartResponse<String> saveFlowRole(String submitDatas,String id) throws Exception {
		SmartResponse<String> smartResp = new SmartResponse<String>();
		if(StringUtils.isNotEmpty(submitDatas) && StringUtils.isNotEmpty(id)) {
			String[] values = submitDatas.split(",");
			smartResp = userServ.addFlowRole2User(id, values);
			values = null;
			submitDatas = null;
		}
		return smartResp;
	 ...
	 * @return
	 */
	@RequestMapping(value="/deleteFlowRole", produces="application/json;charset=UTF-8")
	@ResponseBody
	public SmartResponse<String> deleteFlowRole(String userId, String id) {
	    return userServ.deleteFlowRole(userId, id);
	}
	...
	@RequestMapping("/view")
	public ModelAndView view(HttpSession session,UserSearchParam searchParam,ModelAndView modelView,RequestPage page) throws Exception {
		String uri = "user/view"; 
		addBtn = new EditBtn("add","showPage/base_user_add", null, "添加用户", "600");
		editBtn = new EditBtn("edit","showPage/base_user_edit", "user", "修改用户信息", "600");
		delBtn = new DelBtn("user/delete", "确定要删除选中的用户吗？",uri,null, null);
		...
		String groupId = orgServ.getBusinessGroupByOrgId(user.getOrgId());
		List<TNFontLine> tnflist = fontlineServ.findByGroupId(groupId);
		if(tnflist !=null) {
			List<String> userlist = new ArrayList<>();
			for(TNFontLine  tnf: tnflist) {
				userlist.add(tnf.getUserId());
			}
			searchParam.setUserIds(StringUtils.list2Array(userlist));
		}

//		TNOrg og = orgDao.getSupportDeptIdByOrgName("一线值班人");
//		searchParam.setOrgId(og.getId());
//		searchParam.setOrgIds(StringUtils.list2Array(null));
		pageParam = new PageParam(uri, null, page.getPage(), page.getPageSize());
