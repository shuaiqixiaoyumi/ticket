...
public class RoleService extends MgrServiceImpl<TNRole> {
	...
	@Autowired
	private DocResourceDao docResDao;
	 ...
	 */
	public SmartResponse<String> save(TNRole role,String[] menuIds,List<TreeProp> resAuths, List<TreeProp> docresAuths, String[] docIds) throws ServiceException {
			...
			if(null != role) {
				...
				if(null != id && StringUtils.isNotEmpty(id.toString())) {
					...
					if(null != docIds && docIds.length>0 ) {
						List<TNDoc> docs = docDao.find(docIds);
						if(null != docs && docs.size()>0) {
							isMenu = roleDocDao.save(id.toString(), docs);
							if(!isMenu)
								roleDao.delete(role.getId());
						}
						docs = null;
					...
					if(null != docresAuths && docresAuths.size()>0 && is && isMenu) {
					  	List<TNDoc> resources = docServ.treeProp2Res(docresAuths);
					  	docresAuths = null;
					  	if(null != resources && resources.size()>0) {
					  		isRes = docResDao.save(id.toString(),resources);
					  		if(!isRes) {
					  			roleDao.delete(role.getId());
					  			if(null != roleMenuDao) {
					  				roleMenuDao.delete(menuIds);
	 ...
	 * @throws ServiceException
	 */
	public SmartResponse<String> update(TNRole role,String[] menuIds,List<TreeProp> resAuths,List<TreeProp> docresAuths,String[] docIds) throws ServiceException {
		SmartResponse<String> smartResp = new SmartResponse<String>();
		try {
			if(null != role) {
				if(roleDao.update(role)) {
					...
					if(null != docIds && docIds.length>0) {
						if(roleDocDao.deleteByRole(role.getId())) {
							List<TNDoc> docs = docDao.find(docIds);
							is = roleDocDao.save(role.getId(), docs);
							docs = null;
						}
					} else {
						roleDocDao.deleteByRole(role.getId());
					}
					...
					if(null != docresAuths && docresAuths.size()>0) {
						if(docResDao.deleteByRole(role.getId())) {
						  	List<TNDoc> resources = docServ.treeProp2Res(docresAuths);
						  	docresAuths = null;
						  	if(null != resources && resources.size()>0) {
						  		is = docResDao.save(role.getId(),resources);
						  	}
						  	resources = null;
						}
					} else {
						docResDao.deleteByRole(role.getId());
					}
					if(is) {
						smartResp.setResult(OP_SUCCESS);
						smartResp.setMsg(OP_SUCCESS_MSG);
	...
	public String getRoleIdByRoleName(String roleName) {
		String roleId = "";
		roleId = roleDao.getRoleIdByRoleName(roleName);
		return roleId ;
	}
